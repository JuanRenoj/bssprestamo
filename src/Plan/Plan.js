...
import { Totalfinal, TotalIteres } from '../Funciones/Funciones';
function Plan(props)  {
...
var myInput = document.getElementById("exampleModal");
    ...
    const Busqueda =(e)=>{
      ...
      setdatos(encontrado.filter(function(item){
          return String(item.monto).includes(text)||  item.estado.toLowerCase().includes(text) ;   
...
{/**modal para ingreso de empleado */}
  ...
  <div className="form-outline mb-4">
       <label className="form-label" htmlFor="form1Example1" >No. Cuotas</label>
      ...
      { datos ?
           datos.map((item,index) =>(
            <tr key={index}>
               ...
               <td>{item.interes+"%"}</td>
               <td>{item.plan_dia}</td>
               <td>{Quetzal(TotalIteres(item.monto,item.interes,item.plan_dia)) }</td>
               <td>{Quetzal(Totalfinal(item.monto,TotalIteres(item.monto,item.interes,item.plan_dia))) }</td>
